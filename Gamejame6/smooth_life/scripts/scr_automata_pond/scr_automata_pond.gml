...
function pond_choose_to_die(){
	mapData2 = ds_grid_create(width, height);

	for (var j = 0; j < height; j++) {
	    for (var i = 0; i < width; i++) {
			neighbourWallTiles = pondGetSurroundingWallCount (i, j, 1, 0);
...
function pondGetSurroundingWallCount(_x, _y, view_sizeL, view_sizeS){
	var wallCount = 0;
	var wall_value = 0;
